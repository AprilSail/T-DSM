...
class BernCorrupter(object):
    ...
    def corrupt(self, src, rel, dst, time):
...
class BernCorrupterMulti(object):
    ...
    def corrupt(self, src, rel, dst, time, keep_truth=True):
        n = len(src)
        prob = self.bern_prob[rel]
        ...
        time_out = time.unsqueeze(1).expand(n, self.n_sample)
        if keep_truth:
        ...
        return torch.from_numpy(src_out), rel_out, torch.from_numpy(dst_out), time_out
